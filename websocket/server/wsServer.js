...
exports.WebSocketServer = class extends WebSocket.Server {
  ...
  messageHandler = (e) => {
    console.info("服务端-接收客户端消息")
    const data = JSON.parse(e)
    switch (data.ModeCode) {
      case "message":
        console.log(
          `服务端-收到-${this.ws[this.clientKey]}-消息-${data.msg},已返回`
        )
        this.send(this.ws, data)
        break
      case "heart_beat":
        console.log(`服务端-收到-${this.ws[this.clientKey]}-心跳-${data.msg}`)
        this.send(this.ws, data)
        break
    }
  }
  ...
  errorHandler = (e) => {
    ...
    console.info("服务端-客户端出错")
  }

  closeHandler = (e) => {
    this.removeClient(this.ws)
    console.info("服务端-客户端已断开")
  }

  addClient = (item) => {
    //设备上线时添加到客户端列表
    if (this.webSocketClient[item[this.clientKey]]) {
      console.log(item[this.clientKey] + "-服务端-客户端已存在")
      this.webSocketClient[item[this.clientKey]].close()
    }
    console.log(item[this.clientKey] + "-服务端-客户端已添加")
  ...
  removeClient = (item) => {
    //设备断线时从客户端列表删除
    if (!this.webSocketClient[item[this.clientKey]]) {
      console.log(item[this.clientKey] + "-服务端-客户端不存在")
      return
    }
    console.log(item[this.clientKey] + "-服务端-客户端已移除")
